...
def test_groupby_tabular_count(
    ...
    ds = ray.data.from_items([{"A": (x % 3), "B": 1 if x < 50 else 2} for x in xs]).repartition(
        num_parts
    )
    ds2 = ray.data.from_items([{"A": months[i], "B": score[i]} for i in range(len(months))]).repartition(10)
    if ds_format == "pandas":
        ds = _to_pandas(ds)
    agg_ds = ds.groupby(["A", "B"]).count()
    agg_ds2 = ds2.groupby(["A", "B"]).aggregate(Count(), Sum("B"))
    ...
    agg_ds3 = ds2.groupby("A").sum("B")

    agg_ds4 = ds2.repartition(30).sum("B")

    assert list(agg_ds2.sort([("A", "descending"), ("B", "ascending")]).iter_rows()) == [
        {"A": "September", "B": 1, "count()": 7, "sum(B)": 7},
        {"A": "September", "B": 2, "count()": 3, "sum(B)": 6},
        {"A": "October", "B": 3, "count()": 10, "sum(B)": 30},
        {"A": "December", "B": 1, "count()": 12, "sum(B)": 12},
        {"A": "December", "B": 2, "count()": 8, "sum(B)": 16},
    ]

    assert list(agg_ds2.sort([("count()", "descending")], False).iter_rows()) == [
        {"A": "December", "B": 1, "count()": 12, "sum(B)": 12},
        {"A": "October", "B": 3, "count()": 10, "sum(B)": 30},
        {"A": "December", "B": 2, "count()": 8, "sum(B)": 16},
        {"A": "September", "B": 1, "count()": 7, "sum(B)": 7},
    ...
    assert list(agg_ds3.sort("A", True).iter_rows()) == [
        {"A": "September", "sum(B)": 13},
        {"A": "October", "sum(B)": 30},
        {"A": "December", "sum(B)": 28},
    ]
...
@pytest.mark.parametrize("num_parts", [1, 2, 3, 30])
def test_multi_groupby_map_groups_for_pandas(ray_start_regular_shared, num_parts):
    df = pd.DataFrame({"A": "a a b b".split(), "Next": "1 1 1 1".split(), "B": [1, 1, 3, 5], "C": [4, 6, 5, 5]})
    grouped = ray.data.from_pandas(df).repartition(num_parts).groupby(["A", "Next"])
    ...
    expected = pd.DataFrame(
        {"A": ["a", "a", "b", "b"], "Next": ["1", "1", "1", "1"], "B": [0.5, 0.5, 0.375, 0.625], "C": [0.4, 0.6, 0.5, 0.5]}
    )
    assert mapped.to_pandas().equals(expected)
